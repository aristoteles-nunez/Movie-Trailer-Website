...
def read_data_from_json_file(file_name):
	...
	Args:
		file_name: Exactly path of the file, if the file is in the same directory of the program
			just pass the name.
	Returns:
		A python Dict Object from a valid Json Document.
	Raises:
		An Exception if the data being deserialized is not a valid JSON document or the file
		isn't found.
	"""
...
def main():
	...
	for element in favorites_movies:
		...
		movie = media.Movie(title,storyline,poster_image_url,trailer_youtube_url, release_year,imdb_data_title, imdb_alt_msg)
		movies.append(movie)
